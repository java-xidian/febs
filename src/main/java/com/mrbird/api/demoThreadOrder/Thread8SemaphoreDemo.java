...
/**
 * 8.使用Sephmore(信号量)实现线程按顺序运行 (一手交钱一手交货)
 * Sephmore(信号量):Semaphore是一个计数信号量,从概念上将，
 * Semaphore包含一组许可证,如果有需要的话，每个acquire()方法都会阻塞，直到获取一个可用的许可证,每个release()方法都会释放持有许可证的线程，并且归还Semaphore一个可用的许可证。然而，实际上并没有真实的许可证对象供线程使用，Semaphore只是对可用的数量进行管理维护。
 ...
 * @Date: 2019/4/30
 */
public class Thread8SemaphoreDemo {
