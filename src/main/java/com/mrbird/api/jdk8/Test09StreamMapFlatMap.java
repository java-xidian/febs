...
public class Test09StreamMapFlatMap {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("-----1.map和flatmap的区别----- ");
        list.stream()
        ...
        System.out.println("---2.实际上返回的类似是不同的------- ");
        List<Stream<String>> listResult = list.stream()
                .map(item -> Arrays.stream(item.split(" ")))
                .distinct()
                .collect(Collectors.toList());
        List<String> listResult2 = list.stream()
                .flatMap(item -> Arrays.stream(item.split(" ")))
                .distinct()
                .collect(Collectors.toList());
        //也可以这样
        ...
        System.out.println("====================4.实际上返回的类似是不同的============================");
